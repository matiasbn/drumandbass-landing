...
**Admin & campaigns:** `/admin/junglists` (`api/admin/junglists`, GET list + DELETE, admin-only) — table with search, sort, CSV export, and per-row delete; linked from the admin menu. **The list shows the full junglist universe = `junglists` ∪ `pk_profiles` (DJs)**, because a DJ is *always* a junglist (DJ ⊃ junglist) but lives in `pk_profiles`, not `junglists`. The GET merges both, **deduplicated by email** (if an email is in both, it's a **DJ** — the DJ wins), and tags each row `isDj` so the UI differentiates them (DJ badge; DJ rows show the presskit name/instagram, aren't deletable here — deletion would only touch `junglists`, and a DJ has no row there). DJ name/instagram come from `presskits` (`artist_name`, `socials`). Junglists are also a selectable **email-campaign audience** (`api/admin/campaigns`, audience key `junglists`), unioned and **deduplicated by email** with ravers/registered/DJs, so an email in several lists is sent once. The campaign step-1 UI shows a live "total correos únicos" as audiences are toggled.
...
Email campaigns live in `admin/campaigns/CampaignsClient.tsx` + `api/admin/campaigns/route.ts` (send/list/detail/resend/sync) and send via **Resend** (`resend.batch.send`, max 100/batch; free tier 100/day, 3000/month). Two tables: `campaigns` (one row per send) and `campaign_recipients` (one row per destinatario, with a uuid PK that **is** the tracking token). Recipients are **segmented** (`junglist` / `no_junglist`) so each half gets its own copy (e.g. discount vs. no discount); what actually went out per segment is persisted in `campaigns.segment_content JSONB` so resends replay it **verbatim** (edited drafts used to live only in localStorage and were lost).

**Recipient state has two axes — delivery vs. interaction:**
- **Delivery (hard data, from Resend):** `sent` → `delivered` / `bounced` / `complained`, read live via the `/emails` **list** endpoint (paginated, matched by `resend_id`, cheap) and written back to `status`. Auto-runs when a campaign detail is opened; `failed` = never left (hit the daily quota → resend with "Enviar los que faltan"). Resend's `opened`/`clicked` are folded into `delivered` (they prove delivery) but never used as engagement.
- **Interaction (the only real signal, OUR token):** `visited` = clicked the email button and hit `/evento/[id]?ct=<recipient_id>` → `CampaignVisitBeacon` → `mark_campaign_visit`. **The open pixel was removed entirely** (unreliable): no `/api/campaign-open`, no `mark_campaign_open`, no `opened_at`. "Primer click" per campaign is **derived** (`MIN(visited_at)`), not stored.

**Postgres gotcha — a function that references a dropped/renamed column fails ENTIRELY, silently.** `mark_campaign_visit` once had a second `UPDATE campaigns SET first_click_at=…`; when that column was dropped, the whole function threw on every call, so even the *first* `UPDATE` (the visit) never ran — visits silently stopped saving while an unrelated write (coupon copy) kept working. If some `SECURITY DEFINER`/RPC write mysteriously stops, `SELECT prosrc FROM pg_proc WHERE proname='…'` and check it doesn't touch a column that no longer exists.

**Generic actions log — extend this, don't add per-action columns.** For any campaign-type-specific interaction beyond visits, record it in **`campaign_actions`** (`recipient_id` FK, `action` TEXT, `at`, `meta` JSONB; unique `(recipient_id, action)` = first occurrence). New campaign types add new `action` names with **no schema change**. Recorded from the public landing via `POST /api/campaign-action { ct, action, meta? }` → `record_campaign_action` (SECURITY DEFINER, anon-writable, guarded). This log is **recipient-scoped** (tied to a campaign `ct`), so use it only for things that are genuinely about *this campaign send*. (Currently it has no active writer — coupon copies moved to the event-scoped model below.)

**Coupon copies are tracked by EVENT + USER, not by campaign.** The coupon lives on the event, and copying requires a logged-in junglist, so we always know *who* copied — no `ct` needed. `EventCouponBlock.copy()` calls `record_coupon_copy(event_id)` (SECURITY DEFINER, uses `auth.uid()`, resolves the email from `auth.users`) → one row per `(event_id, user_id)` in **`event_coupon_copies`**, regardless of whether they arrived from a campaign, another campaign, or organically. The campaign detail's "Copió código" column joins `event_coupon_copies` (by `event_id` + lowercased email) against the recipients, so it shows whether each person copied the code **by any path** — not just via that campaign. (This is why the campaign visit `ct` sessionStorage-through-OAuth hack was removed: the copier is authenticated, so attribution is trivial.)

**Parent/child lineage (re-engagement).** Resends are child campaigns: `campaigns.parent_campaign_id` points at the **root** (children are flat siblings, not chained). The detail's "Insistir a los que no interactuaron" button creates `<root> · Reenvío N` targeting everyone who received the root but visited **no** email in the whole line (root + all children), minus bounced/spam and unsubscribed — computed by `computeResendTargets`, identical no matter which campaign in the line you trigger it from.
...
**The codes are secret — never let them reach the client.** They're served ONLY by `get_event_coupon(event_id)` (SECURITY DEFINER, decides new/existing from `auth.uid()` + `coupon_set_at`, returns the applicable code or NULL). The code never goes in public HTML or the email — the email links to the landing, which reveals it against session + junglist status. **Column-grant gotcha (security-critical):** `cms_events` uses **per-column SELECT grants** so `anon`/`authenticated` can read everything EXCEPT `coupon_junglist_new` / `coupon_junglist` (they used to be readable with the public key — a real leak that was fixed). The public read gets booleans `has_coupon_new` / `has_coupon_existing` (generated columns) for the "Descuento Junglist" badge; `lib/cms.ts` maps those to `couponForNew`/`couponForExisting`. **Consequences (both bit us — do not repeat):**
- **NEVER `select('*')` on `cms_events`** — with the code columns revoked, `*` expands to all columns and Postgres denies the whole query (`42501 permission denied for table cms_events`). This breaks reads (home, admin list) AND `.insert(...).select()` / `.update(...).select()` (the returning clause is also `*`). Use the explicit **`CMS_EVENT_SELECT`** constant (`lib/cms.ts`) everywhere — `getEvents`/`getEventById`, `api/admin/events` GET, and the insert/update returning selects all use it.
- **Any NEW column added to `cms_events` must be added to BOTH** the `GRANT SELECT (...)` list (`supabase/migrations/20260730000000_hide_event_coupon_codes.sql`) **and** `CMS_EVENT_SELECT`, or it's invisible to the front.
- Filtering/ordering by the code columns from `anon`/`authenticated` also fails (Postgres needs SELECT to reference a column in `WHERE`) — filter by the booleans instead (see `api/junglist/discounts`).

**Editing coupons (two entry points, event is the source of truth):** the coupon is edited from **`/admin/eventos`** (a "Descuento Junglist" section, two code fields) OR configured in the campaign flow (`applyEventTemplate` **prefills** from the event if it already has one). Both write the codes to `cms_events`. Because the codes aren't SELECT-able anymore (the security fix above), reading them back for the edit form / campaign prefill goes through **`admin_event_coupons(event_id)`** (SECURITY DEFINER, admin-gated) exposed as `GET /api/admin/events?couponFor=<id>`. Writes normalize codes to UPPERCASE and set `coupon_set_at` **once** (never moved).
