...
Gotchas: the helper only fires in prod (dev has no `gtag`). Server components can't call `event()` directly — wrap the interactive element in a small `'use client'` component (see `src/components/TicketButton.tsx`). GA aggregates custom events by **param value**, not by any DB/Contentful entry id, so recycled Contentful event entries measure fine as long as `event_title`/`event_url` stay unique per occurrence.
