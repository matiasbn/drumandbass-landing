...
**Generic actions log — extend this, don't add per-action columns.** For any campaign-type-specific interaction beyond visits, record it in **`campaign_actions`** (`recipient_id` FK, `action` TEXT, `at`, `meta` JSONB; unique `(recipient_id, action)` = first occurrence). New campaign types add new `action` names with **no schema change**. Recorded from the public landing via `POST /api/campaign-action { ct, action, meta? }` → `record_campaign_action` (SECURITY DEFINER, so anon can write; guarded by recipient existence + `ON CONFLICT DO NOTHING`). First use: `coupon_copy` (fired from `EventCouponBlock` when the discount code is copied), surfaced in the campaign detail as "Copiaron el código: N · 1ª vez …". **Don't reach for a new column when a new `action` fits** — that's the whole point of the model.
