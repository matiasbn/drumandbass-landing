...
**Admin analytics dashboard:** `/admin/analytics` (`admin/analytics/AnalyticsClient.tsx`) renders a native dashboard from the **GA4 Data API** (`src/lib/ga.ts` + `api/admin/analytics`, admin-gated) — scorecards, daily active users, top pages, events, channels, and ticket clicks per event. Credentials: `GA_SERVICE_ACCOUNT_KEY` (service-account JSON, base64) + `GA_PROPERTY_ID`. Falls back to a Looker Studio embed (`NEXT_PUBLIC_LOOKER_STUDIO_URL`) when the service account isn't configured. **Ticket clicks are attributed to an event by TITLE + the DAY the click happened — not by `event_date`.** `TicketButton` sends `event_title` + `event_date` + `event_url` with each `event_link_click` (the `event_date` param is still sent but the dashboard no longer needs it registered). The dashboard queries GA by `customEvent:event_title` × the **native `date` dimension** (when the click occurred) and, in `api/admin/analytics/route.ts`, assigns each click to the **next occurrence of that title on/after the click day** (people click "Tickets" *before* the event). Since two same-title events always have **distinct dates**, the click-day → next-occurrence rule separates them unambiguously; and because the panel only lists **vigent (upcoming) events**, a past occurrence's clicks are attributed to that past event and simply not shown — they never leak into the current same-title event. This is why we dropped the earlier title+`event_date` scheme: `event_date` custom dimensions are **not retroactive** (historical clicks would show `(not set)`), whereas title + native click-day works with existing data. **This needs ONE GA4 custom dimension** (event-scoped): `event_title`, registered in GA4 Admin → Custom definitions (already registered as "Titulo del evento" → param `event_title`). Without it the panel is empty and shows the fallback subtitle. `ga.ts` returns `ticketClickRows` (title × click-day); the route does the windowing and fills `ticketClicks`. The only case this can't split is two same-title events on the **exact same date** — which per the events' nature never happens; if it ever did, register `event_date` and key on title+date instead.
