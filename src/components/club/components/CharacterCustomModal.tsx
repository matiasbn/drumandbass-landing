...
const FACE_LABELS = ['Redonda', 'Cruz', 'Linea', 'Curva'];

// Same SVGs as useFaceTexture — inlined for preview rendering
const FACE_SVGS = [
...
interface CharacterCustomModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
function renderFacePreview(
  ...
  const bgColor = DEFAULT_BG_COLORS[hashColor % DEFAULT_BG_COLORS.length];
...
export const CharacterCustomModal: React.FC<CharacterCustomModalProps> = ({ isOpen, onClose }) => {
  const { profile, updateProfile } = useAuth();
  const username = profile?.username || '';

  // Compute hash-based defaults
  const defaultColor = React.useMemo(() => {
    if (!username) return NEON_COLORS[0];
    let hash = 0;
  ...
  const defaultFaceType = React.useMemo(() => {
    if (!username) return 0;
    return stringHash(username + '_face') % FACE_SVGS.length;
  }, [username]);
  ...
  // Render face previews
  useEffect(() => {
    if (!isOpen || !username) return;
    canvasRefs.current.forEach((canvas, i) => {
      if (canvas) renderFacePreview(canvas, i, username);
    });
  }, [isOpen, username]);
      ...
      <div className="relative w-full max-w-md mx-4 bg-[#0a0a0a] border border-white/20 shadow-2xl">
        ...
        <div className="p-6 space-y-6">
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: selectedColor === color
                      ? `0 0 0 3px #0a0a0a, 0 0 0 5px ${color}, 0 0 15px ${color}`
                      : 'none',
            ...
            <div className="flex gap-3">
              {FACE_LABELS.map((label, i) => (
                <button
                  ...
                  style={{
                    borderColor: selectedFace === i ? selectedColor : 'rgba(255,255,255,0.15)',
                    boxShadow: selectedFace === i ? `0 0 10px ${selectedColor}44` : 'none',
                  }}
                >
                  <canvas
                    ref={setCanvasRef(i)}
                  ...
                  />
                  <span className="text-white/50 font-mono text-[9px]">{label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className="w-full py-3 font-mono text-sm tracking-wider transition-all bg-[#ff0055] text-white hover:bg-[#ff0055]/80 disabled:opacity-50"
          >
